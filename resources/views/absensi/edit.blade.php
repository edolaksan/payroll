...
<div class="container mx-auto mt-10">
    <h2 class="text-2xl font-semibold mb-4">Edit Absensi Karyawan</h2>
    ...
    <form action="{{ route('absensi.update', $absensi->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="pegawai_id" id="pegawai_id" class="w-full px-3 py-2 border border-gray-300 rounded" required>
                @foreach ($pegawai as $p)
                    <option value="{{ $p->id }}" {{ $p->id == $absensi->pegawai_id ? 'selected' : '' }}>
                        {{ $p->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="time" name="jam_masuk" id="jam_masuk" value="{{ $absensi->jam_masuk }}" class="w-full px-3 py-2 border border-gray-300 rounded" required>
        </div>

        <div class="mb-4">
            <label for="jam_pulang" class="block text-gray-700 font-medium mb-1">Jam Pulang</label>
            <input type="time" name="jam_pulang" id="jam_pulang" value="{{ $absensi->jam_pulang }}" class="w-full px-3 py-2 border border-gray-300 rounded">
        </div>
